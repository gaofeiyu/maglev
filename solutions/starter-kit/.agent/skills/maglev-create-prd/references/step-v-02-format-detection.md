...
检测 PRD 是否遵循 Maglev 格式并进行适当路由 - 分类为 Maglev 标准 / Maglev 变体 / 非标准，并对非标准格式进行可选的平价检查。
...
### 2. 检查 Maglev PRD 核心部分

检查 PRD 是否包含以下 Maglev PRD 核心部分：
...
**Maglev 变体：**
- 3-4 个核心部分存在
- 通常遵循 Maglev 模式，但可能有结构差异
- 缺少某些部分但可识别为 Maglev 风格
...
**格式分类:** [Maglev 标准 / Maglev 变体 / 非标准]
**存在的核心部分:** [count]/6
```

### 5. 根据格式分类进行路由

**如果格式是 Maglev 标准或 Maglev 变体：**
...
此 PRD 不遵循 Maglev 标准结构（仅存在 {count}/6 个核心部分）。
...
**[A] 平价检查** - 分析差距并估算达到 Maglev PRD 平价的工作量
**[B] 原样验证** - 使用当前结构继续验证
**[C] 退出** - 退出验证并审查格式发现
