...
**🌟 关键结构校验 (Three-Zone Architecture):**
*   **Zone 1: Executive Brief (决策层)**: 顶部必须有简明的摘要、核心决策和显性风险。纯人话，无技术堆砌。
    > *Prompt*: "Imagine briefing the CEO in an elevator."
*   **Zone 2: Logic Core (执行层)**: 文档主体，包含用户旅程、功能需求等。
*   **Zone 3: Context Dungeon (上下文层)**: 原始 Prompt 或思维链必须折叠在 `<details>` 中。
...
**必须保留：**
- **Zone 1 的 Executive Brief (如果没有，请根据内容生成一个)**
- 所有用户成功标准
- 所有功能需求（能力合同）
- 所有用户旅程叙事
- 所有范围决策（MVP、增长、愿景）
